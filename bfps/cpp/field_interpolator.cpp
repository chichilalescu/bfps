...
#include <cmath>
#include "field_interpolator.hpp"

template <class rnumber,
          int interp_neighbours,
          field_backend be,
          field_components fc>
field_interpolator<rnumber,
                   interp_neighbours,
                   be,
                   fc>::field_interpolator(
        field_layout<fc> *FLAYOUT,
        base_polynomial_values BETA_POLYS)
{
    // generate compute array
    this->flayout = new field_layout<fc>(
            FLAYOUT->sizes,
            FLAYOUT->subsizes,
            FLAYOUT->starts,
            FLAYOUT->comm);
...
template <class rnumber,
          ...
          field_backend be,
          field_components fc>
bool field_interpolator<
    rnumber,
    interp_neighbours,
    be,
    fc>::get_rank_info(double z, int &maxz_rank, int &minz_rank)
{
    bool is_here = false;
    if (be == FFTW)
    {
        int zg = int(floor(z/this->dz));
        minz_rank = this->flayout->rank[MOD(
                 zg - interp_neighbours,
                this->flayout->sizes[0])];
        maxz_rank = this->flayout->rank[MOD(
                zg + 1 + interp_neighbours,
                this->flayout->sizes[0])];
        for (int iz = -interp_neighbours; iz <= interp_neighbours+1; iz++)
            is_here = (is_here ||
                       (this->flayout->myrank ==
                        this->flayout->rank[MOD(zg+iz, this->flayout->sizes[0])]));
    }
    return is_here;
}
...
void field_interpolator<rnumber, interp_neighbours>::operator()(
        const int *xg,
        const double *xx,
        double *dest,
        const int *deriv)
...
template class field_interpolator<float, 1>;
template class field_interpolator<float, 2>;
template class field_interpolator<float, 3>;
template class field_interpolator<float, 4>;
template class field_interpolator<float, 5>;
template class field_interpolator<float, 6>;
template class field_interpolator<double, 1>;
template class field_interpolator<double, 2>;
template class field_interpolator<double, 3>;
template class field_interpolator<double, 4>;
template class field_interpolator<double, 5>;
template class field_interpolator<double, 6>;
